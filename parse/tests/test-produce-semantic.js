...
const repoBase = path.resolve(__dirname, '..', '..'); // repo root two levels up
...
/**
 * Run semantic() over the full input and return rich token objects.
 * @param {string} markdown
 */
function parseAndGetSemanticTokens(markdown) {
  /** @type {number[]} */
  const semanticTokens = [];
  const scan = semantic({ input: markdown, startOffset: 0, endOffset: markdown.length });
  scan(semanticTokens);

  let pos = 0;
  return semanticTokens.map(token => {
    const kind = getTokenKind(token);
    const length = getTokenLength(token);
    const flags = getTokenFlags(token) | (token & HeadingDepthMask);
    const text = markdown.slice(pos, pos + length);
    const offset = pos;
    pos += length;
    return { offset, length, kind, flags, text };
  });
}

/**
 * Find .md files (non-recursive) in a directory.
 * @param {string} dir
